...
export class TelegramControlService {
  ...
  async extractAndProcessMessages(
    ...
    this.logger.log(
      `Extrayendo y procesando mensajes para el canal: ${channelName}`,
    );
    ...
    try {
      ...
      if (!channelConfig) {
        this.logger.error(
          `No se encontró la configuración para el canal con ID: ${channelId}`,
        );
        return;
      }
      ...
      const newMessages = await this.fetchMessagesFromTelegram(
        channelName,
      ...
      if (newMessages && newMessages.length > 0) {
        ...
        const filteredMessages = await this.handleCacheMessages(
          newMessages,
          channelName,
        );
        ...
        this.logger.log(
          `Se procesaron ${filteredMessages.length} mensajes para el canal ${channelName}.  Nuevo lastExtractedMessageId: ${lastMessageId}`,
        );
      } else {
        this.logger.log(
      ...
      this.logger.error(
        `Error al extraer y procesar mensajes para el canal ${channelName}: ${error.message}`,
        error.stack,
      );
  ...
  private async fetchMessagesFromTelegram(
    ...
    return await this.client.getMessages(channelName, {
      limit,
      reverse: true,
      minId: offsetId,
    });
  ...
  private async handleCacheMessages(
    ...
    console.log(`Hashes generados para ${messageHashes.length} mensajes.`);
    ...
    console.log(`Mensajes existentes encontrados: ${existingMessages.length}`);

    const existingHashes = new Set(
      existingMessages.map((msg) => msg.messageHash),
    );
    ...
    for (const message of messages) {
      ...
      try {
        ...
        if (
          existingHashes.has(hashedMessage) ||
          processedHashes.has(hashedMessage)
        ) {
        ...
        cachedMessage.authorId = message.fromId
          ? JSON.stringify(message.fromId)
          : 'unknown';
        cachedMessage.fromClass = message.fromId
          ? message.fromId.className
          : 'unknown';
        ...
        this.logger.log(
          `Guardando mensaje con id ${message.id} y hash ${hashedMessage}`,
        );
        await this.cachedMessageRepository.save(cachedMessage);
        processedHashes.add(hashedMessage);

        filteredMessages.push(message);
      } catch (error) {
        this.logger.error(
          `Error procesando mensaje con id ${message.id}: ${error.message}`,
          error.stack,
        );
  ...
  private extractOwnerId(message: Api.Message): bigInt.BigInteger | undefined {
    if (message.fromId?.className == 'PeerUser') return message.fromId.userId;
      ...
      return message.fromId.channelId;
    else if (message.fromId?.className == 'PeerChat')
      return message.fromId.chatId;
  }
}
